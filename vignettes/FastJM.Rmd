...
mdata <- mydata$mdata
```
The number of rows in ydata is the total number of measurements for all subjects in the study. The columns in ydata should start with the subject ID (column 1), longitudinal outcome, the covariates for the random effects, and then the covariates for the fixed effects. For cdata, the survival / censoring time is included in the first column, and the failure type coded as 0 (censored events), 1 (risk 1), or 2 (risk 2) is given in the second column. Either competing risks or single failure assumed. The covariates are included in the third column and on. mdata is to indicate the number of longitudinal measurements per subject. The number of rows equals to the number of subjects.
