...
Rcpp::List getmvCov_lm(const Eigen::VectorXd beta, 
                       ...
                       Rcpp::List bList, Rcpp::List XsList, double s, Eigen::MatrixXd Zs,  std::string latAsso) {
  ...
  Eigen::VectorXd N = Eigen::VectorXd::Zero(numBio);
  Eigen::VectorXd TN = Eigen::VectorXd::Zero(numBio);
  Eigen::VectorXd TN1 = Eigen::VectorXd::Zero(numBio);
  Eigen::VectorXd TRN = Eigen::VectorXd::Zero(numBio);
  Eigen::VectorXd TRNN = Eigen::VectorXd::Zero(numBio);
  Eigen::MatrixXd TRNN1 = Eigen::MatrixXd::Zero(numBio,numSubj);
  Eigen::MatrixXd TRNN2 = Eigen::MatrixXd::Zero(numBio,numSubj);
  Eigen::MatrixXd TNN1 = Eigen::MatrixXd::Zero(numBio,a);
  Eigen::MatrixXd TNN11 = Eigen::MatrixXd::Zero(numBio,a);
  Eigen::MatrixXd TNN2 = Eigen::MatrixXd::Zero(numBio,b);
  Eigen::MatrixXd TNN22 = Eigen::MatrixXd::Zero(numBio,b);
  ...
  Eigen::MatrixXd BAssociation = Eigen::MatrixXd::Zero(numBio,numBio);
  
  
  Eigen::MatrixXd FUNEC = Eigen::MatrixXd::Zero(2, numSubj); //exp(alpha*b)
  //     //bexp(alpha*b)
  Eigen::MatrixXd FUNBEC = Eigen::MatrixXd::Zero(2*numBio,numSubj); // 2 for number of competing risks
  // 
  // 
  // //     //bbTexp(alpha*b)
  // Eigen::MatrixXd FUNBSEC = Eigen::MatrixXd::Zero(2*pRE*(pRE+1)/2,k);
  
  Eigen::VectorXd FUNBECvec = Eigen::VectorXd::Zero(2*numBio);
  // 
  // 
 ...
 // 1 x pg
      Eigen::VectorXd betag = beta.segment(betaIndex, p);           // pg x 1
      Eigen::VectorXd bVecig = bVeci.segment(reIndex, pRE);                   // qg x 1
      
      
      Eigen::RowVectorXd Zg_s = Zs.block(g, reIndex, 1, pRE);              // 1 x qg
      
      // latent(i, g) = (Xg_s * betag)(0) + (Zg_s * bVecig)(0);
  ...
  for(int i = 0; i < numSubj; i++){
    ...
    BAssociation = Zs*sigi*Zs.transpose(); // this part need ot make slexible
    
    Eigen::MatrixXd FUNBSmat =  sigi + MultVVoutprod(bVeci); 
    // Eigen::MatrixXd FUNBSmat =  sigi + MultVVoutprod(latent_i); // pREtotalxpRetotal
    ...
    FUNEC(0,i) = exp(alpha1.dot(latent_i) + 0.5 * tausq);
    FUNBEC.col(i).segment(index, numBio) = exp(MultVV(alpha1, latent_i) + 0.5 * tausq) * (BAssociation * alpha1 + latent_i);
    FUNBEC.col(i).segment(index, numBio) = exp(alpha1.dot(latent_i) + 0.5 * tausq) *(BAssociation * alpha1 + latent_i);
    
    // 
    tausq = alpha2.transpose() * BAssociation * alpha2;
    FUNEC(1,i) = exp(alpha2.dot(latent_i) + 0.5 * tausq);
    FUNBEC.col(i).segment(index + numBio, numBio) = exp(MultVV(alpha2, latent_i) + 0.5 * tausq) * (BAssociation * alpha2 + latent_i);
  }
  ...
  for (int i = 0; i < numSubj; i++) {
    ...
    for(int g = 0; g < numBio; g++){
      ...
      for (int nij = 0; nij < numRep; nij++) {
        ...
        Eigen::MatrixXd sigig = sigi.block(bIndex, bIndex, pRE, pRE); // THIS BLOCK NEEDS TO make sure it's subject specific
    ...
    if (i == 0){
      ...
      TN = Eigen::VectorXd::Zero(numBio);
      TRN = Eigen::VectorXd::Zero(numBio);
      
      risk1_index=risk1_index_vtemp;
      for (i2=i;i2<numSubj;i2++){
        Eigen::MatrixXd sigi = Rcpp::as<Eigen::MatrixXd>(sigmaiList[i2]);
        BAssociation = Zs*sigi*Zs.transpose();
        Eigen::VectorXd latent_i = latent.row(i2).transpose();
        
        tausq = alpha1.transpose() * BAssociation * alpha1;
        
        temp += exp(MultVV(W.row(i2), gamma1))*FUNEC(0, i2);
        
        N = FUNBEC.col(i2).segment(0, numBio);
        ...
        if (cmprsk(i2) == 1)
        {
          ...
          else
          {
            ...
            {
              temp += exp(MultVV(W.row(i2), gamma1))*FUNEC(0, i2);
              for (int ind=0;ind<numBio;ind++) N(ind) = FUNBEC(ind,i2);
    ...
    if (i==0)
    {
      for (t=0;t<numBio;t++) N(t) = FUNBEC(t,i);
    ...
    if (survtime(i) >= H01(risk1_index_vtttemp,0))
    {
      if (cmprsk(i) == 1)
      {
        TN = latent.row(i).transpose() - TNN11.col(a-1-risk1_index_vtttemp);
        TN += TRN;
        
        for (q=0;q<numBio;q++) S(index + q) = TN(q);
        
      }
      else
      {
        for (q=0;q<numBio;q++) S(index + q) = TRN(q);
      }
    }
    else
    {
    ...
    if (i == 0)
    {
      temp=0;
      TN = Eigen::VectorXd::Zero(numBio);
      TRNN = Eigen::VectorXd::Zero(numBio);
      ...
      for (i2=i;i2<numSubj;i2++)
      {
        ...
        for (int ind=0;ind<numBio;ind++) N(ind) = FUNBEC(numBio+ind,i2);
    ...
    if (i==0)
    {
      for (t=0;t<numBio;t++) N(t) = FUNBEC(numBio+t,i);
    ...
    if (survtime(i) >= H02(risk2_index_vtttemp,0))
    {
      if (cmprsk(i) == 2)
      {
        TN = latent.row(i).transpose()- TNN22.col(b-1-risk2_index_vtttemp);
        TN += TRN;
        for (q=0;q<numBio;q++) S(index + q) = TN(q);
      }
      else
  ...
  Eigen::VectorXd sealpha1 = Eigen::VectorXd::Zero(numBio);
  Eigen::VectorXd sealpha2 = Eigen::VectorXd::Zero(numBio);
  ...
  // alpha
  for (t=0;t<numBio;t++) sealpha1(t) = sqrt(SSinv(index+t,index+t));
  index += numBio;
  for (t=0;t<numBio;t++) sealpha2(t) = sqrt(SSinv(index+t,index+t));
  index += numBio;
