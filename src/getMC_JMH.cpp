...
//
// [[Rcpp::export]]
Rcpp::List getMC_JMH(Eigen::VectorXd & beta, Eigen::VectorXd & tau, 
                 Eigen::VectorXd & gamma1, Eigen::VectorXd & gamma2, 
                 Eigen::VectorXd & alpha1, Eigen::VectorXd & alpha2, 
                 double vee1, double vee2, Eigen::MatrixXd & H01, 
                 Eigen::MatrixXd & H02, Eigen::MatrixXd & Sig, 
  ...
  for (j=0;j<k;j++)
  {
    G=exp(MultVV(X2.row(j), gamma2))*FUNWSEC(1,j);
    TG+=G;
    V=exp(MultVV(X2.row(j), gamma2))*FUNWEC(1,j);
    TV+=V;
    ...
    if (cmprsk(j) == 2)
    {
      if (j == k-1)
      {
        TG*=H02(risk2_index,2);
        TGG+=TG;
        TG/=H02(risk2_index,2);
        TV*=H02(risk2_index,2);
        TVV+=TV;
        TV/=H02(risk2_index,2);
        risk2_index--;
