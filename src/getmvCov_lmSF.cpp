...
Rcpp::List getmvCov_lmSF(const Eigen::VectorXd beta, 
  ...
  Eigen::MatrixXd FUNEC = Eigen::MatrixXd::Zero(1, numSubj); //exp(alpha*b)
  //     //bexp(alpha*b)
  Eigen::MatrixXd FUNBEC = Eigen::MatrixXd::Zero(1*numBio,numSubj); //  for number of competing risks
  // 
  // 
  // //     //bbTexp(alpha*b)
  // Eigen::MatrixXd FUNBSEC = Eigen::MatrixXd::Zero(2*pRE*(pRE+1)/2,k);
  
  Eigen::VectorXd FUNBECvec = Eigen::VectorXd::Zero(numBio);
  // 
  // 
 ...
 // 1 x pg
  ...
  for(int i = 0; i < numSubj; i++){
    ...
    FUNBEC.col(i) = exp(alpha1.dot(latent_i) + 0.5 * tausq) * (BAssociation * alpha1 + latent_i);
    // 
  }
