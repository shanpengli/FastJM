...
//
// [[Rcpp::export]]
Rcpp::List getCov_JMH(const Eigen::VectorXd & beta, const Eigen::VectorXd & tau, 
  ...
  int d = beta.size() + W.cols() + 2*gamma1.size() + 2*alpha1.size() +
    2 + Sig.cols()*(Sig.cols() + 1)/2;
  int a = H01.rows();
  int b = H02.rows();
  int k = mdata.size();
  ...
  int risk2_index_wtemp=b-1;
  int risk2_index_wttemp=b-1;
  int risk2_index_wtttemp=b-1;
  ...
  Eigen::MatrixXd SXX2 = Eigen::MatrixXd::Zero(p2, b);
  Eigen::MatrixXd SXX22 = Eigen::MatrixXd::Zero(p2, b);
  ...
  Eigen::MatrixXd TNN2 = Eigen::MatrixXd::Zero(p1a,b);
  Eigen::MatrixXd TNN22 = Eigen::MatrixXd::Zero(p1a,b);
  ...
  Eigen::VectorXd TGG2 = Eigen::VectorXd::Zero(b);
  Eigen::VectorXd TGG22 = Eigen::VectorXd::Zero(b);
  ...
  for (j=0;j<k;j++) {
    ...
    if (j == 0)
    {
      ...
      SRXX = Eigen::VectorXd::Zero(p2);
      risk2_index=risk2_index_temp;
      for (q=j;q<k;q++)
      {
        
        temp+=exp(MultVV(X2.row(q), gamma2))*FUNEC(1,q);
        SX += exp(MultVV(X2.row(q), gamma2))*FUNEC(1,q)*X2.row(q);
    ...
    else
    {
      ...
      {
        if (survtime(j) >= H02(risk2_index_temp, 0))
        {
          SRXX2.col(j) = SRXX2.col(j-1);
        }
        else
        {
          risk2_index_temp--;
    ...
    if (j==0)
    {
      SRX -= CumuH02(risk2_index_ttemp)*X2.row(j);  
    }
    else if (survtime(j) >= H02(risk2_index_ttemp, 0))
    {
      SRX -= CumuH02(risk2_index_ttemp)*X2.row(j);  
    }
    else
    ...
    SRX*= exp(MultVV(X2.row(j), gamma2))*FUNEC(1,j);
    
    if (survtime(j) >= H02(risk2_index_tttemp, 0))
    {
      if (cmprsk(j) == 2)
      {
        X = X2.row(j);
        ...
        X += SRX;
        for (q=0;q<p2;q++) S(p1+p1b+p2+q) = X(q);
      }
      else
      {
        for (q=0;q<p2;q++) S(p1+p1b+p2+q) = SRX(q);
      }
    }
    else
    {
    ...
    if (survtime(j) >= H01(risk1_index_vtttemp,0))
    {
      if (cmprsk(j) == 1)
      {
        ...
        TN += TRN;
        for (q=0;q<p1a;q++) S(p1+p1b+2*p2+q) = TN(q);
      }
      else
      {
        for (q=0;q<p1a;q++) S(p1+p1b+2*p2+q) = TRN(q);
      }
    }
    else
    {
    ...
    if (j == 0)
    {
      temp=0;
      TN = Eigen::VectorXd::Zero(p1a);
      TRNN = Eigen::VectorXd::Zero(p1a);
      risk2_index=risk2_index_vtemp;
      
      for (q=j;q<k;q++)
      {
        temp += exp(MultVV(X2.row(q), gamma2))*FUNEC(1, q);
        for (i=0;i<p1a;i++) N(i) = FUNBEC(p1a+i,q);
        TN += exp(MultVV(X2.row(q), gamma2))*N;
        if (cmprsk(q) == 2)
        {
          if (q == k-1)
          {
    ...
    else
    {
      ...
      {
        if (survtime(j) >= H02(risk2_index_vtemp, 0))
        {
          TRNN2.col(j) = TRNN2.col(j-1);
        }
        else
        {
          risk2_index_vtemp--;
    ...
    TRN *= exp(MultVV(X2.row(j), gamma2))*FUNEC(1,j);
    
    if (j==0)
    {
      for (t=0;t<p1a;t++) N(t) = FUNBEC(p1a+t,j);
      N *= CumuH02(risk2_index_vttemp)*exp(MultVV(X2.row(j), gamma2));
      TRN -= N;
    }
    else if (survtime(j) >= H02(risk2_index_vttemp,0))
    {
      for (t=0;t<p1a;t++) N(t) = FUNBEC(p1a+t,j);
      N *= CumuH02(risk2_index_vttemp)*exp(MultVV(X2.row(j), gamma2));
      TRN -= N;
    ...
    if (survtime(j) >= H02(risk2_index_vtttemp,0))
    {
      if (cmprsk(j) == 2)
      {
        TN = FUNB.col(j) - TNN22.col(b-1-risk2_index_vtttemp);
        TN += TRN;
        for (q=0;q<p1a;q++) S(p1+p1b+2*p2+p1a+q) = TN(q);
      }
      else
      {
        for (q=0;q<p1a;q++) S(p1+p1b+2*p2+p1a+q) = TRN(q);
      }
    }
    else
    {
    ...
    if (survtime(j) >= H01(risk1_index_wtttemp,0))
    {
      if (cmprsk(j) == 1)
      {
        ...
        TG += TRG;
        S(p1+p1b+2*p2+2*p1a) = TG;
      }
      else
      {
        S(p1+p1b+2*p2+2*p1a) = TRG;
      }
    }
    else
    {
    ...
    if (j == 0)
    {
      ...
      risk2_index=risk2_index_wtemp;
      for (q=j;q<k;q++)
      {
        temp += exp(MultVV(X2.row(q), gamma2))*FUNEC(1, q);
        G = FUNWEC(1,q);
        TG += exp(MultVV(X2.row(q), gamma2))*G;
        if (cmprsk(q) == 2)
        {
          if (q == k-1)
          {
            TG *= H02(risk2_index, 1)/pow(temp,2);
            TGG2(b-1-risk2_index) = TG;
            TRGG += TG;
            TG /= H02(risk2_index, 1)/pow(temp,2);
            TG /= temp;
            TGG22(b-1-risk2_index) = TG;
            TG *= temp;
            risk2_index--;
          }
          else if (survtime(q+1) != survtime(q))
    ...
    {
      if (risk2_index_wtemp>=0)
      {
        if (survtime(j) >= H02(risk2_index_wtemp, 0))
        {
          TRGG2(j) = TRGG2(j-1);
        }
        else
        {
          ...
          {
            TRGG = TRGG2(j-1);
            TRGG -= TGG2(b-1-risk2_index_wtemp-1);
            TRGG2(j) = TRGG;
          }
        }
      }
    ...
    TRG *= exp(MultVV(X2.row(j), gamma2))*FUNEC(1,j);
    
    if (j==0)
    {
      G = FUNWEC(1,j);
      G *= CumuH02(risk2_index_wttemp)*exp(MultVV(X2.row(j), gamma2));
      TRG -= G;
    }
    else if (survtime(j) >= H02(risk2_index_wttemp,0))
    {
      G = FUNWEC(1,j);
      G *= CumuH02(risk2_index_wttemp)*exp(MultVV(X2.row(j), gamma2));
      TRG -= G;
    ...
    else
    {
      ...
      {
        risk2_index_wttemp=0;
      }
    }
    
    
    if (survtime(j) >= H02(risk2_index_wtttemp,0))
    {
      if (cmprsk(j) == 2)
      {
        TG = FUNW(j) - TGG22(b-1-risk2_index_wtttemp);
        TG += TRG;
        S(p1+p1b+2*p2+2*p1a+1) = TG;
      }
      else
      {
        S(p1+p1b+2*p2+2*p1a+1) = TRG;
      }
    }
    else
    {
      risk2_index_wtttemp--;
      if (risk2_index_wtttemp>=0)
      {
        if (cmprsk(j) == 2)
        {
          TG = FUNW(j) - TGG22(b-1-risk2_index_wtttemp);
  ...
  Eigen::VectorXd segamma2 = Eigen::VectorXd::Zero(p2);
  Eigen::VectorXd sealpha1 = Eigen::VectorXd::Zero(p1a);
  Eigen::VectorXd sealpha2 = Eigen::VectorXd::Zero(p1a);
  double sevee1=0;
  double sevee2=0;
  ...
  for (t=0;t<p2;t++) segamma2(t) = sqrt(SSinv(p1+p1b+p2+t,p1+p1b+p2+t));
  for (t=0;t<p1a;t++) sealpha1(t) = sqrt(SSinv(p1+p1b+2*p2+t,p1+p1b+2*p2+t));
  for (t=0;t<p1a;t++) sealpha2(t) = sqrt(SSinv(p1+p1b+2*p2+p1a+t,p1+p1b+2*p2+p1a+t));
