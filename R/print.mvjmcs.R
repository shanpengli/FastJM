  ...
  if (x$CompetingRisk) {
    ...
    for(g in 1:numBio){
      ...
      if (pRE == 1){
        tempName[ind] <- paste0("(Intercept)_1","bio",g)
          ...
          tempName[ind:(ind+pRE-1)] <- c(temp, paste0(all.vars(x$random[[g]])[-pRE],"_1","_bio", g))
      }  
      ind <- ind + pRE
    }
    ...
    for(g in 1:numBio){
      ...
      if (pRE == 1){
        ...
        tempName[ind:(ind+pRE-1)] <- c(temp, paste0(all.vars(x$random[[g]])[-pRE],"_2","_bio", g))
      }  
      ind <- ind + pRE
    ...
    cat("\nRandom effects:                 \n")
    for(g in 1:numBio){
      cat("  Formula ",g, ": ", format(as.formula(x$random[[g]])), "\n")
    }
    ...
    pREtotal <- length(diag(x$Sig))
      ...
      for(g in 1:numBio){
        pRE <- pREvec[g]
        re[pREind:(pREind+pRE-1)] <- paste0(all.vars(x$random[[g]]),g)
        pREind <- pREind+pRE
      }
