GetEmpiricalCIF <- function(data, time, status) {
  ...
  fit <- survival::survfit(survival::Surv(time, status3) ~ 1, data = data)
  table <- summary(fit)
  ...
  ## NA estimate of H1(t)
  fit <- survival::survfit(survival::Surv(time, status1) ~ 1, data = data)
  tableH1 <- summary(fit)
  ...
  ## NA estimate of H2(t)
  fit <- survival::survfit(survival::Surv(time, status2) ~ 1, data = data)
  tableH2 <- summary(fit)
