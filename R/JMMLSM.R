...
##' fixed-effect covariates in the longitudinal mean submodel.
##' @param surv.formula A formula object specifying the survival time, event
##' indicator, and covariates in the survival submodel.
##' @param variance.formula A one-sided formula object specifying the fixed-effect
##' covariates in the within-subject variance submodel.
...
##'   Available options are \code{"adaptive"} and \code{"standard"}. The default
##'   is \code{"adaptive"}.}
...
JMMLSM <- function(cdata, ydata,
  ...
  getinit <- Getinit.JMH(cdata = cdata, ydata = ydata, long.formula = long.formula,
                     ...
                     model = model, ID = ID, RE = RE, random = random, survinitial = TRUE,
  ...
  if (CompetingRisk == TRUE) {
    ...
    if (iter == maxiter) {
      ...
      getloglike <- getLoglike.JMH(beta, tau, gamma1, gamma2, alpha1, alpha2, vee1, vee2, 
                               ...
                               mdataS, xsmatrix, wsmatrix, method, initial.optimizer = "BFGS")
    ...
    repeat
    {
      ...
      if (method == "standard") {
        ...
        GetEfun <- GetESFad.JMH(beta, tau, gamma1, alpha1, vee1, H01, Sig, Z, X1, W, Y, X2, 
                            survtime, cmprsk, mdata, mdataS, xsmatrix, wsmatrix, initial.optimizer = "BFGS")
      } else {
        stop("Please choose one of the following methods for numerical integration in the E-step: standard, adaptive.")
      }
      ...
      if((DiffSFrelative.JMH(beta, prebeta, tau, pretau, gamma1, pregamma1,
                 alpha1, prealpha1, vee1, prevee1, Sig, preSig, H01, preH01, tol) == 0) 
    ...
    if (iter == maxiter) {
      ...
      getloglike <- getLoglikeSF.JMH(beta, tau, gamma1, alpha1, vee1, H01, Sig, Z, X1, W, Y, 
                                 X2, survtime, cmprsk, mdata, mdataS, xsmatrix, wsmatrix, method, initial.optimizer = "BFGS")
