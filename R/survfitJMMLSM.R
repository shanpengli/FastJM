...
survfitJMMLSM <- function(object, seed = 100, ynewdata = NULL, cnewdata = NULL, 
                          ...
                          method = c("Laplace", "GH"), quadpoint = NULL, ...) {
  ...
  getdum <- getdummy.JMH(long.formula = object$LongitudinalSubmodelmean,
      ...
      if (method == "GH") {
        for (jj in 1:lengthu) {
          ...
          if (quadmethod == "standard") {
            CIF <- getECIF_JMH(beta, tau, gamma1, gamma2, alpha1, alpha2, nu1,
            ...
            opt <- optim(rep(0, nsig), logLikCR.JMH, data = data, method = "BFGS", hessian = TRUE)
            Posmean <- opt$par
            PosCov <- solve(opt$hessian)
            
            CIF <- getECIFad_JMH(beta, tau, gamma1, gamma2, alpha1, alpha2, nu1,
                             nu2, Sig, Z, X, W, Y, as.vector(X2), H01, H02,
        ...
        for (jj in 1:lengthu) {
          ## calculate the CIF
          CIF1 <- CIF1.CR.JMH(data, H01, H02, s, u[jj], meanbw)
          P1us <- Pk.us.JMH(CIF1, data, meanbw)
          Predraw1[j, jj] <- P1us
          CIF2 <- CIF2.CR.JMH(data, H01, H02, s, u[jj], meanbw)
          P2us <- Pk.us.JMH(CIF2, data, meanbw)
          Predraw2[j, jj] <- P2us
        }
        quadpoint = NULL
      }
    ...
    for (j in 1:N.ID) {
      ...
      if (method == "Laplace") {
        ...
        for (jj in 1:lengthu) {
          ...
          if (quadmethod == "standard") {
            Predraw[j, jj] <- getES_JMH(beta, tau, gamma, alpha, nu, Sig, Z, X, W, Y, 
            ...
            Predraw[j, jj] <- getESad_JMH(beta, tau, gamma, alpha, nu, Sig, Z, X, W, Y, 
