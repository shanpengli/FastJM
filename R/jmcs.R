...
##' survival time, event indicator, and baseline covariates.
##' @param long.formula A formula object specifying the response variable and
##' fixed-effect covariates in the longitudinal submodel.
##' @param random A one-sided formula object specifying the random-effects
##' structure in the longitudinal submodel. For example, a random-intercept model
##' can be specified as \code{~ 1 | ID}, and a random-intercept and random-slope
##' model can be specified as \code{~ x1 + ... + xn | ID}.
...
##' \code{\link{jmcs_control}()}. Available options include:
##' \describe{
##'   \item{\code{quadpoint}}{Number of pseudo-adaptive or standard
##'   Gauss--Hermite quadrature points used for numerical integration. The default is \code{6}.}
##'   \item{\code{maxiter}}{Maximum number of EM iterations. The default is \code{10000}.}
...
##'   \item{\code{initial.para}}{Optional list of user-supplied initial
##'   parameter values for the EM algorithm. The default is \code{NULL}.}
##'   \item{\code{tol}}{Convergence tolerance for the EM algorithm. The default is \code{1e-4}.}
##'   \item{\code{method}}{Numerical integration method used in the E-step.
##'   Available options are \code{"pseudo-adaptive"} and \code{"standard"}.
##'   The default is \code{"pseudo-adaptive"}.}
##'   \item{\code{opt}}{Optimization method used to fit the initial linear
##'   mixed-effects model. Available options are \code{"nlminb"} and
##'   \code{"optim"}. The default is \code{"nlminb"}.}
##' }
...
jmcs <- function(ydata, cdata, long.formula, random = NULL, surv.formula,
                 control = jmcs_control()) {
  
  control <- utils::modifyList(jmcs_control(), control)
  quadpoint    <- control$quadpoint
